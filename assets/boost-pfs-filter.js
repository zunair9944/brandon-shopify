...
ProductGridItem.prototype.compileTemplate = function(data) {
  ...
  // Custom fields by avex
  try {
    itemHtml = itemHtml.replace(/{{itemBadge}}/g, buildItemBadge(data));
    itemHtml = itemHtml.replace(/{{itemSubtitle}}/g, buildItemSubtitle(data));
    itemHtml = itemHtml.replace(/{{itemProductCardImages}}/g, buildItemProductCardImages(data));
    itemHtml = itemHtml.replace(/{{wishlistIcon}}/g, buildWishlistIcon(data));
    itemHtml = itemHtml.replace(/{{itemHasHoverImage}}/g, buildHasHoverImageState(data));
  } catch (ex) {
    console.error(ex);
  }
  // End of custom fields by avex
...
// Custom methods by avex
function buildItemBadge(productData) {
  let badge = '';

  const collectionData = JSON.parse(document.querySelector('.collection__data, .search__data')?.innerHTML || '{}');

  let isComingSoon = false;
  const isSoldOut = productData.available === false;
  let customBadge = '';

  productData.tags.forEach(tag => {
    if (tag.includes('COMING SOON')) {
      isComingSoon = true;
    } else if (tag.includes('badge::')) {
      customBadge = tag.replace('badge::', '');
    }
  })

  if (isComingSoon) {
    badge = collectionData.t.coming_soon;
  }  else if (isSoldOut) {
    badge = collectionData.t.sold_out;
  } else {
    badge = customBadge;
  }
...
function buildItemSubtitle(productData) {
  let subtitle = '';
  const firstVariantOptionColor = productData?.variants[0]?.title || productData?.variants[0]?.option_color;
  if (firstVariantOptionColor && firstVariantOptionColor !== 'Default Title') subtitle = firstVariantOptionColor;
  return subtitle;
}

function buildWishlistIcon(productData) {
  let result = '';
  if (productData?.id) { 
    result += `
    <button 
      data-with-epi="true"
      class="swym-button product-section__wishlist-btn product_${ productData.id }" 
      data-swaction="addToWishlist" 
      data-product-id="${ productData.id }" 
      data-variant-id="${productData.variants[0].id}" 
      data-product-url="${window.location.protocol + window.location.hostname + Utils.buildProductItemUrl(productData)}"></button>`
    ; 
  }
  return result
}

function buildItemProductCardImages(productData) {
  let result = '';

  const featuredImage = productData?.images_info[0];
  if (featuredImage) {
    result += `<img 
      src="${featuredImage.src}&amp;width=512" 
      alt="${featuredImage.alt || ''}" 
      srcset="${featuredImage.src}&amp;width=512 512w" 
  ...
  const hoverImage = productData?.images_info?.find(image => image?.alt?.includes('|| hover')) || productData?.images_info[1];
  if (hoverImage) {
    result += `<img 
      src="${hoverImage.src}&amp;width=512" 
      alt="${hoverImage.alt || ''}" 
      srcset="${hoverImage.src}&amp;width=512 512w" 
...
function buildHasHoverImageState(productData) {
  let result = false;
  
  const hoverImage = productData?.images_info?.find(image => image?.alt?.includes('|| hover')) || productData?.images_info[1];
  if (hoverImage) result = true;

  return result;
}
// End of custom methods by avex
...
Filter.prototype.afterRender = function(data) {
  ...
  // *********
  // *********
  // Avex custom elements
  renderFeedInlineImage();
  watchFilterBarStickyState();
  modifyFilterTemplate();
  // End of avex custom elements
  // *********
  // *********
...
function renderFeedInlineImage() {
  const feedInlineImageClass = 'collection__feed-inline-image-box';
  
  const content = document.querySelector('.collection__content, .section-search');
  const { feedInlineImage, feedInlineImageTitle, feedInlineImageBtnLabel, feedInlineImageBtnUrl } = content?.dataset || {};
  const existingFeedInlineImage = content?.querySelector(`.${feedInlineImageClass}`);
  if (existingFeedInlineImage || !feedInlineImage) {
    // feed inline image was already rendered
    // or feed inline image is not defined for this collection
    return;
  }
  
  const pfsContainer = content.querySelector('.collection__content-pfs');
  const pfsProducts = pfsContainer.querySelector('.boost-pfs-filter-products');
  if (!pfsProducts) {
    console.warn('Products list not found');
    return;
  }
  const targetElement = pfsProducts.querySelector('.product-card:nth-child(12), .boost-pfs-filter-product-item:nth-child(12)');
  if (!targetElement) {
    console.warn('Not enough products in the collection to render "feed inline image"');
    return;
  }
  const feedInlineImageElement = document.createElement('div');
  feedInlineImageElement.classList.add(feedInlineImageClass, ...targetElement.classList);
  feedInlineImageElement.innerHTML = `
    <div class="collection__feed-inline-image-content" data-has-link="${feedInlineImageBtnUrl ? true : false}">
      ${feedInlineImageBtnUrl ? `<a href="${feedInlineImageBtnUrl}" class="collection__feed-inline-image-btn-url"></a>` : ''}
      
      <div class="collection__feed-inline-image-copy">
        ${feedInlineImageTitle ? `<span class="collection__feed-inline-image-title">${italicizeFirstWord(feedInlineImageTitle)}</span>` : ''}
        ${feedInlineImageBtnLabel ? `
          <span class="collection__feed-inline-image-btn">
            <span class="collection__feed-inline-image-btn-title text-1">${feedInlineImageBtnLabel}</span>
            <span class="collection__feed-inline-image-btn-icon">
              ${THEME_GLOBAL.ICON_SVG.RIGHT_LONG}
            </span>
          </span>
        ` : ''}
      </div>

      <img class="collection__feed-inline-image" src="${feedInlineImage}" height="540" alt="" />
    </div>
  `;
  targetElement.parentNode.insertBefore(feedInlineImageElement, targetElement.nextSibling);
}

function watchFilterBarStickyState() {
  const content = document.querySelector('.collection__content, .section-search');
  
  const { isWatchingStickyState } = content.dataset;
  if (isWatchingStickyState === 'true') return;

  content.dataset.isWatchingStickyState = 'true';
  const filterBar = content.querySelector('.boost-pfs-filter-tree-h-wrapper');
  const filterBarMobile = content.querySelector('.boost-pfs-filter-toolbar-top-mobile');

  function updateFilterBarStickyState(filterBar, targetDevice) {
    const filterBarParent = filterBar.parentElement;
    const otherStickyContentHeight = THEME_GLOBAL.DIMENSION.headerHeight || 0;
    const { top, bottom } = filterBarParent.getBoundingClientRect();

    let filterBarHeight = filterBar.offsetHeight;
    if (targetDevice === 'mobile') {
      filterBarHeight = filterBar.querySelector('.boost-pfs-filter-toolbar-top-mobile-inner').offsetHeight;
    }

    filterBarParent.style.setProperty('--sticky-offset-top', `${otherStickyContentHeight}px`);
    filterBarParent.style.setProperty('--filter-bar-height', `${filterBarHeight}px`);
    if (top < otherStickyContentHeight && bottom > otherStickyContentHeight) {
      filterBarParent.dataset.filterBarIsSticky = 'true';
    } else {
      filterBarParent.dataset.filterBarIsSticky = 'false';
    }
  }

  function runUpdate() {
    if (window.innerWidth > THEME_GLOBAL.BREAKPOINT.TABLET) {
      updateFilterBarStickyState(filterBar, 'desktop');
    } else {
      updateFilterBarStickyState(filterBarMobile, 'mobile');
    }
  }
...
function modifyFilterTemplate() {
  const collectionData = JSON.parse(document.querySelector('.collection__data, .search__data')?.innerHTML || '{}');
  const content = document.querySelector('.collection__content, .section-search');
  const filterTemplate = content.querySelector('.collection__content-pfs, .search__data');

  // const filterButtonTexts = filterTemplate.querySelectorAll('.boost-pfs-filter-tree-mobile-button button > span, .boost-pfs-filter-tree-content .boost-pfs-filter-mobile-toolbar-header');
  // filterButtonTexts.forEach(filterButtonText => {
  //   filterButtonText.innerText = collectionData.t.filters;
  // })

  // Replaces the sort button's text to "sort" 
  const sortingButtonText = filterTemplate.querySelector('.boost-pfs-filter-top-sorting-wrapper span > span');
  if (sortingButtonText) sortingButtonText.innerText = collectionData.t.sort;
}
