...
(function() {
  ...
  // hides blog posts and shows only pages
  InstantSearchResultItemPage.prototype.getTemplate = function () {
    let isPage = this.data.url.includes('pages');
    if(!isPage) return ''
    return `
  ...
  InstantSearchResult.prototype.afterRender = function() {
    jQ(this.$instantSearchResult).find('[aria-label="Collections"]').find('.boost-pfs-search-suggestion-item.boost-pfs-ui-item > a').each(function() {
        var text = jQ(this).text();
        text = text.replace('prodGroup::', '');
        jQ(this).text(text);
    })
  };
